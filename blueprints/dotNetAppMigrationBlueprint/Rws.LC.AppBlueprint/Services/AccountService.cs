...
namespace Rws.LC.AppBlueprint.Services
{
    ...
    public class AccountService : IAccountService
    {
        ...
        /// <inheritdoc/>
        public async Task SaveRegistrationInfo(RegisteredUpdatedEvent registeredEvent, CancellationToken cancellationToken)
        {
        ...
        /// <inheritdoc/>
        public async Task UpdateToDescriptorVersion14(RegisteredUpdatedEvent updatedEvent, CancellationToken cancellationToken)
        {
            if (string.IsNullOrEmpty(updatedEvent.ClientCredentials?.ClientId))
            {
                throw new AppValidationException($"Invalid {nameof(updatedEvent.ClientCredentials.ClientId)} provided.", new Details { Code = ErrorCodes.InvalidInput, Name = nameof(updatedEvent.ClientCredentials.ClientId), Value = null });
            }

            if (string.IsNullOrEmpty(updatedEvent.ClientCredentials?.ClientSecret))
            {
                throw new AppValidationException($"Invalid {nameof(updatedEvent.ClientCredentials.ClientSecret)} provided.", new Details { Code = ErrorCodes.InvalidInput, Name = nameof(updatedEvent.ClientCredentials.ClientSecret), Value = null });
            }

            AppRegistrationEntity appRegistrationEntity = new AppRegistrationEntity()
            {
                ClientCredentials = updatedEvent.ClientCredentials.ToEntity()
            };

            await _appRegistrationRepository.SaveRegistrationInfo(appRegistrationEntity).ConfigureAwait(false);

            await _repository.RemoveAccountsClientCredentials().ConfigureAwait(false);
        }
