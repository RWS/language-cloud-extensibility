...
namespace Rws.LC.AppBlueprint.DAL
{
    public class Repository : IRepository
    {
        ...
        /// <summary>
        /// Removes all the client credentials per accounts.
        /// This is invoked when the app is updated to descriptorVersion 1.4 since it will use the "global" credentials from the registration repo.
        /// </summary>
        /// <returns></returns>
        public async Task RemoveAccountsClientCredentials()
        {
            string unsetQuery = "{ $unset: { ClientCredentials: 1 } }";

            var emptyFilter = FilterDefinition<AccountInfoEntity>.Empty;
            var update = BsonDocument.Parse(unsetQuery);

            await _accounts.UpdateManyAsync(emptyFilter, update).ConfigureAwait(false);
        }
